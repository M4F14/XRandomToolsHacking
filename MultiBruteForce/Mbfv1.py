	...
	while 1:
		...
		if v:
			if a.upper() in v:
				...
				tampil('\rm[!]Masukan Opsinya Goblok...')
				continue
		else:
			if len(a) == 0:
				tampil('\rm[!]Masukin dengan bener bego')
				continue
			else:
				break
	return a
...
def idteman():
	...
	try:
		...
		if len(id_teman) != 0:
			tampil('\rm[!]Sory, Hanya dapat mengambil \rp%d id'%len(id_bteman))
		else:
			tampil('\rm[!]Batal')
			keluar()
	while 1:
...
def menu():
	tampil('''\rh
           ...
           {\rmX\rh}v{\rmX\rh}|\rcPRX\rh|==========.
             [']  /'|'\           \\
                 /  \  \           '
                 \_  \_ \_    \rk*\rhSCARY OF DEMON CYBER
\rk###########################################################
#             \rb*MULTI BRUTEFORCE FACEBOOK*\rk                 #
# \rhBY\rp                                             X-RANDOM1971 \rk#
# \rhGroup FB\rp  https://m.facebook.com/groups/164201767529837 \rk#
# \rhGitHub\rp                      https://github.com/M4F14 \rk#
#       \rmJangan Di Pake Buat Meras Harta Orang Ya          \rk#
###########################################################''')
